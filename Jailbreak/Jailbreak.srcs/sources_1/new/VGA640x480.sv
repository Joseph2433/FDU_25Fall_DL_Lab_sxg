 module VGA640x480( 
    ...
    // derived constants
    localparam 
      H_SYNC_START = H_SYNC + H_BACK,                       //????????? = 144(96+48)
      H_SYNC_END   = H_SYNC + H_BACK + H_DISPLAY,           //???????? = 784(96+48+640)
      H_PIXELS     = H_SYNC + H_BACK + H_DISPLAY + H_FRONT, //????????? = 800(96+48+640+16)
      V_SYNC_START = V_SYNC + V_TOP,                        //????????? =  31(2+29)
      V_SYNC_END   = V_SYNC + V_TOP + V_DISPLAY,            //???????? = 511(2+29+480)
